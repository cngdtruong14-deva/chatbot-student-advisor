...
param(
    ...
    [string]$DatasetName = 'questions_v3_reviewed.jsonl',
...
if (-not (Test-Path -LiteralPath (Join-Path $chroma '.benchmark-disposable'))) {
    throw 'DisposableChromaDirectory must be a copied index containing .benchmark-disposable; the active index must never be mounted.'
}
if (Test-Path -LiteralPath $OutputDirectory) {
    if (@(Get-ChildItem -LiteralPath $OutputDirectory -Force).Count -ne 0) { throw 'OutputDirectory must be new or empty; final runs are never resumed or overwritten.' }
} else {
...
# The key is read by Docker from the operator-owned env file and is never
# interpolated, printed, copied into the output directory, or written to logs.
$args = @(
...
& docker @args
if ($LASTEXITCODE -ne 0) { throw "RAG Benchmark V3 container failed with exit code $LASTEXITCODE" }
