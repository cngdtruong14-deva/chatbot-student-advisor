"""Freeze an owner-reviewed V3 dataset and create its line-by-line ledger.

The input rows must already contain the owner's review decisions.  This tool
does not write questions, gold chunks, answers, or review decisions.
"""
...
from app.prepare_rag_benchmark_v3 import prepare
from app.rag_benchmark_v2 import load_jsonl, validate_benchmark_protocol


CONFIRM = "OWNER_REVIEWED_ALL_120_V3_ROWS"


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--input", type=Path, required=True)
    parser.add_argument("--prior-v2", type=Path, required=True)
    ...
    if args.confirm != CONFIRM:
        raise ValueError("V3_EXPLICIT_OWNER_REVIEW_CONFIRMATION_REQUIRED")
    if args.dataset_output.exists() or args.ledger_output.exists():
        raise ValueError("V3_SIGNED_OUTPUT_EXISTS")
    rows = load_jsonl(args.input)
    prepare([dict(row) for row in rows], load_jsonl(args.prior_v2))  # overlap/leakage recheck
    owner = args.owner.strip()
    if not owner:
        raise ValueError("V3_OWNER_REQUIRED")
    for row in rows:
        if row.get("reviewed") is not True or str(row.get("reviewer") or "").strip() != owner or not row.get("reviewed_at"):
            raise ValueError(f"V3_ROW_NOT_OWNER_REVIEWED:{row.get('question_id')}")
    ...
    print(json.dumps({"status": "OWNER_REVIEW_FROZEN", "question_count": 120, "dataset_canonical_sha256": protocol["dataset_canonical_sha256"]}, ensure_ascii=False))
    return 0
