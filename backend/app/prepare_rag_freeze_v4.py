"""Create an unsigned one-shot V4 Test freeze from reviewed Dev evidence."""
...
def main() -> int:
    ...
    if args.output.exists():
        raise ValueError("V4_FREEZE_TEMPLATE_OUTPUT_EXISTS")
    ...
    if report.get("split") != "dev" or report.get("dataset_canonical_sha256") != protocol["dataset_canonical_sha256"]:
        raise ValueError("V4_FREEZE_DEV_REPORT_DATASET_MISMATCH")
    selected = (report.get("dev_selection") or {}).get("recommended_method")
    if not selected or selected != (binding.get("retriever") or {}).get("method"):
        raise ValueError("V4_FREEZE_DEV_RETRIEVER_NOT_SELECTED")
    quality = report.get("quality") or {}
    if quality.get("infrastructure_error_count") != 0:
        raise ValueError("V4_FREEZE_DEV_INFRASTRUCTURE_ERRORS")
    ...
    if not isinstance(release, dict) or release != expected_release:
        raise ValueError("V4_FREEZE_RELEASE_BINDING_MISMATCH")
    value = {
    ...
    print(json.dumps({"status": "OWNER_FREEZE_APPROVAL_REQUIRED", "benchmark_version": "V4",
